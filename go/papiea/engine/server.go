...
func update_spec(metadata, spec) task,err {
	...
	if (old_spec = cas(metada, spec)) {
		// TODO: optimize number of updates to task
		task.update_spec_delta(compute_delta(old_spec, spec))
		delta = compute_delta(spec, status)
		// analyze_function will sort functions based on the call graph
		function_list = analyze_function(delta);
		provider = get_handling_provider(metadata.kind);
		async invoke_functions(provider, function_list, strategy, task, metadata);
	}
	else {
		task.fail("wrong spec");
	}
	return task;
...
func register_provider_intentful_callback (status_diff_signature, callback_url) {
	diff_analyzer.insert(status_diff_signature, callback_url);
}

func register_provider_procedural_callback (procedure_signature, callback_url) {
...
func provider_facing_api(api) {
	...
	api.post(papiea+"/update_status", func(req, res) {
		res.status(update_status_in_db(req.uuid, req.status) ? 200 : 400);
		return res;
	})
}

func user_facing_api(api) {
	...
	foreach (provider : all_providers) {
		foreach (kind : provider) {
			...
			foreach (procedure : kind.procedures) {
...
func start_main_server(api) {
	admin_facing_api(api);
	
	provider_facing_api(api);
